...
class web:
    ...
    def search(self , api_key , cse_id , search_term, **kwargs):    
        ...
        result = google_service.cse().list(q=search_term, cx=cse_id,
        **kwargs).execute()
    ...
    def get_results(self, api_key , cse_id , search_term):
        ...
        for k in range(0,7):
            res = self.search(api_key , cse_id , search_term , 
                start=k*10+1 , num=10)
            
            for z in res:
                if re.match('https' , z['formattedUrl'])!=None:
                    z['formattedUrl']=re.sub('https','http',
                    z['formattedUrl'])
    ...
    def scrape_site(url):
        ...
        try:
            ...
            # The above line removes content from script tags
            [s.extract() for s in soup('style')] 
            # The above line removes content from style tags
            phrases=[]
            words=[]
            ...
            for k in phrases:
                for z in k:
                    flag=1
                    for ch in z:
                        if re.match('[a-zA-z]',ch)== None: 
                        #Filter out words by checking each character
                            flag=0
                            break
                    if z!='' and flag==1:
                        words.append(z)
